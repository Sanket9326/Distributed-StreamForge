...
export class UploadPage implements OnDestroy {
  ...
  constructor(
    ...
    private readonly uploadCompletions: UploadCompletionService,
  ) {}
  ...
  protected onDragOver(event: DragEvent): void {
    ...
    if (this.state() !== 'uploading') this.isDragging.set(true);
  }
  ...
  protected startUpload(): void {
    ...
    this.uploadSubscription = this.uploadService
      .upload(file, metadataValidation.metadata)
      .subscribe({
        next: (event) => {
          ...
          if (event.type === HttpEventType.Response && event.body) {
            ...
            this.uploadCompletions.track(event.body.id, event.body.title);
          }
        },
  ...
  private validateFile(file: File): string | null {
    ...
    if (!UploadPage.allowedExtensions.includes(extension)) {
    ...
    if (file.size > UploadPage.maxFileSizeBytes) {
  ...
  private validateMetadata(): { metadata: UploadMetadata; error: string | null } {
    ...
    return { metadata: { title, description: description || null, hashtags }, error: null };
  }
